...
func (s S3ReplayStorage) Upload(gZipFilePath, target string) (err error) {
	...
	if err != nil {
		return fmt.Errorf("open %s file failed: %s", gZipFilePath, err)
	}
	defer file.Close()
	s3Config := &aws.Config{
	...
	if err != nil {
		return fmt.Errorf("s3 new session failed: %s", err)
	}
	...
	if err != nil {
		return fmt.Errorf("s3 upload file %s failed: %s", gZipFilePath, err)
	}
